...
async function run() {
  ...
  await withServers({}, async () => {
    ...
    assert.equal(landing.headers.get("x-relayer-release"), process.env.OPENCODE_ROUTER_RELEASE_ID || "v0.2.4")
  ...
  await withServers({ sessionCount: 1 }, async () => {
    ...
    assert.equal(health.data.release.releaseId, process.env.OPENCODE_ROUTER_RELEASE_ID || "v0.2.4")
  ...
  await withServers({}, async () => {
    ...
    assert.notEqual(second.headers.get("x-oc-cache"), "hit")
    assert(["active-session-bypass", "view-session-bypass"].includes(second.headers.get("x-oc-relay-reason")))
  ...
  await withServers({ metaCacheMs: 100, snapshotCacheMs: 100, watchIntervalMs: 200, enableProgressQueryOverride: true }, async () => {
    ...
    assert.equal(override.data.launch.sessionID, "ses_latest")
